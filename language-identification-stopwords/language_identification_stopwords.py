...
if __name__ == "__main__":
    ...
    text_data = tira.pd.inputs("nlpbuw-fsu-sose-24", "language-identification-validation-20240429-training")
    labels_data = tira.pd.truths("nlpbuw-fsu-sose-24", "language-identification-validation-20240429-training")
    ...
    data = text_data.merge(labels_data, on='id')
    ...
    # Predict on the full Tira validation data
    stopword_features_test = stopword_features(text_data['text'], stopwords)
    char_features_test = char_vectorizer.transform(text_data['text'])
    word_features_test = word_vectorizer.transform(text_data['text'])
    additional_features_test = TextFeatures().transform(text_data['text'])
    combined_features_test = hstack([char_features_test, word_features_test, additional_features_test, stopword_features_test])

    test_predictions = clf.predict(combined_features_test)

    # Prepare the predictions DataFrame
    predictions_df = pd.DataFrame({'id': text_data['id'], 'lang': test_predictions})
